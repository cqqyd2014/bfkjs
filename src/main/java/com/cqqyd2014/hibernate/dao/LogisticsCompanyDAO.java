...
public class LogisticsCompanyDAO {
	public com.cqqyd2014.hibernate.entities.LogisticsCompany getObjectById(Session session,String id) throws Exception{
	...
	public java.util.ArrayList<com.cqqyd2014.hibernate.entities.LogisticsCompany> getList(Session session){
		 CriteriaBuilder builder=session.getCriteriaBuilder();    //创建一个Criteria生成器
	        CriteriaQuery<com.cqqyd2014.hibernate.entities.LogisticsCompany> criteria = builder.createQuery(com.cqqyd2014.hibernate.entities.LogisticsCompany.class);    //创建一个CriteriaQuery对象，指定至返回值类型为Person对象
	        Root<com.cqqyd2014.hibernate.entities.LogisticsCompany> root=criteria.from(com.cqqyd2014.hibernate.entities.LogisticsCompany.class);//指定查询的root类型
	        criteria.select(root);//指定返回的结果集中的类型
	...
	public java.util.LinkedHashMap<String, String> getNameMap(Session session){
		java.util.ArrayList<LogisticsCompany> list=getList(session);
	...
	public java.util.LinkedHashMap<String, java.math.BigDecimal> getBillLengthMap(Session session){
		java.util.ArrayList<LogisticsCompany> list=getList(session);
