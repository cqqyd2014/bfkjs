...
public abstract class DeliverListAction extends ActionSupport{
	...
	public java.util.ArrayList<com.cqqyd2014.order.model.DeliverBill> getList() throws Exception {
		...
		try {
			...
			java.util.ArrayList<String> arraylist_delivers=getDelivers(session,start_date,end_date,goods_barcode,deliverbill_status ,express_com,express_no,com_id,rows,receiver_name,receiver_mobile,reciever_addr,page,send_user,create_userid,order_no);
			...
			if (arraylist_delivers.size()==0) {
				...
				dbs=com.cqqyd2014.order.logic.DeliverMLogic.getArrayListModelWithDetail(session, arraylist_delivers, com_id);
			}
